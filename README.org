...
To *execute the cell at your cursor in the Jupyter notebook*, use the command ~jupyter-ascending-execute-line~.

To *execute all cells*, use ~jupyter-ascending-execute-all~.

~jupyter-ascending-next-cell~ and ~jupyter-ascending-previous-cell~ can be used to *move the cursor between cells* or *create a new cell*, in the case of the former command.

With the cursor in a Markdown cell, ~jupyter-ascending-edit-markdown-cell~ can be used to edit the content of that cell in a dedicated Markdown buffer.

~jupyter-ascending-cycle-cell-type~ can be used to toggle between Markdown and code cells.
* Limitations
